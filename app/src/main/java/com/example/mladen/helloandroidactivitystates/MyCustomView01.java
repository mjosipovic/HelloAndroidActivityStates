...
//                    mY = (int) event.getY();
//
//                    mCanvas.drawRect(mX, mY, mX + WIDTH, mY + HEIGHT, mPaint);
...
//                Log.d(DEBUG_TAG, "onTouch: " + event.toString());
                ...
                if (event.getAction() == MotionEvent.ACTION_DOWN) {
                    ...
                    if(tmpDirection != null){
                        drawLinOpArtBetweenPoints(mT1, mT2, tmpDirection);
                        return true;
                    }
                    else{
                        return false;
...
//            Log.d(DEBUG_TAG, "tmpStripeCol="+tmpStripeCol);
    ...
    private void drawLinOpArtBetweenPoints(Point t1, Point t2, MyUtils.MOVE_DIRECTION direction){
        switch (direction) {
            case HORIZONTAL:
